...
- 用 2025-01 至今的数据做 OOS holdout 验证
- 如果 holdout 期 Sharpe 为负或超额收益为负，停下来分析原因是因子失效还是其他问题
- 不通不过：不进入下一阶段

### Caveat: OOS 污染

已知 `12_1 mom` 的选择受到了 2024-2026 数据上 walk-forward 结果（~250% 收益）的影响——研究决策不是盲态的。因此本次 OOS 测试的性质是"参考验证"而非"盲测"，结论用于确认方向，不作为最终有效性证明。真正的盲测需要未来区间。

### Success criteria

- holdout 期 Sharpe > 0（目标 > 0.5）
- holdout 期超额收益（相对 TOPIX）> 0
...
- 基本面因子（可选，视 EDINET 数据获取难度）：
  ...
  - 如果数据清洗量过大：推迟到后续版本
  - **准入条件**：EDINET 数据必须使用公告日（提交日）而非报告期末日期。point-in-time 对齐是硬要求——用报告期日期的数据会引入 look-ahead bias，该项目此前已有类似教训
...
- 风险优化后的组合在 holdout 期 Sharpe 相比等权基线提升 > 0.1，或 max drawdown 减少 > 5 个百分点
